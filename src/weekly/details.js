...
function createCommentArticle(comment) {
  ...
  footer.textContent = 'Posted by: ' +(comment.author || 'Unknown');
...
async function handleAddComment(event) {
  ...
  try {
    const response = await fetch('./api/index.php?action=comment', {
      ...
      body: JSON.stringify({week_id: Number(currentWeekId), author: 'Student', text: commentText})
    });
...
async function initializePage() {
 ...
 currentWeekId = getWeekIdFromURL();
  ...
  try {
    ...
    currentComments = commentsResult.success && Array.isArray(commentsResult.data)? commentsResult.data : [];   
